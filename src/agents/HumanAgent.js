...
export default class HumanAgent {
  ...
  updateState(territory) {
    if (this.gameState === states.INITIAL_ASSIGN && territory.name in this.currentTerritories) {
      territory.addArmy(1);
      this.freeArmies--;
    ...
    } else if (this.gameState === states.ASSIGN_ARMY && territory.name in this.currentTerritories) {
      territory.addArmy(1);
      this.freeArmies--;
      if (this.freeArmies === 0) {
    ...
    } else if (this.gameState === states.ATTACK && territory.name in this.currentTerritories) {
      this.attackingTerritory = territory;
      this.gameState = states.VICTIM;
      return false;

    } else if (this.gameState === states.VICTIM && this.attackingTerritory.isAdjEnemy(territory)) {
      this.defendingTerritory = territory;
      this.performAttack()
      this.gameState = states.ASSIGN_ARMY;
      this.calculateBonusArmy();
  ...
  performAttack() {
    ...
    if (attarmy == defarmy) {
      ...
      this.defendingTerritory.getAgent().removeTerritory(this.defendingTerritory);
      this.currentTerritories[this.defendingTerritory.name] = this.defendingTerritory
